...
void opcode_8XY4(CHIP8 *chip); // Add VY to VX; VF is set to 1 if there's a carry, else 0
void opcode_8XY5(CHIP8 *chip); // Subtract VY from VX; VF is set to 0 if there's a borrow, else 1
void opcode_8XY6(CHIP8 *chip); // Store the least significant bit of VX in VF and then shift VX to the right by 1
void opcode_8XY7(CHIP8 *chip); // Set VX to VY minus VX; VF is set to 0 if there's a borrow, else 1
void opcode_8XYE(CHIP8 *chip); // Store the most significant bit of VX in VF and then shift VX to the left by 1
...
void opcode_CXNN(CHIP8 *chip); // Set VX to the result of a bitwise AND operation on a random number and NN
void opcode_DXYN(CHIP8 *chip); // Draw a sprite at coordinates VX, VY with a width of 8 pixels and a height of N pixels
void opcode_EX9E(CHIP8 *chip); // Skip next instruction if the key stored in VX is pressed
void opcode_EXA1(CHIP8 *chip); // Skip next instruction if the key stored in VX isn't pressed
...
void opcode_FX1E(CHIP8 *chip); // Add the value stored in VX to I; VF is set to 1 if there's a range overflow, else 0
void opcode_FX29(CHIP8 *chip); // Set I to the location of the sprite for the character in VX
void opcode_FX33(CHIP8 *chip); // Store the binary-coded decimal representation of VX at address I, I+1, and I+2
void opcode_FX55(CHIP8 *chip); // Store V0 to VX (inclusive) in memory starting at address I
void opcode_FX65(CHIP8 *chip); // Fill V0 to VX (inclusive) with values from memory starting at address I
